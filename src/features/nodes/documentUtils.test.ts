...
describe('createDocumentNode', () => {
  it('should create a document node with required fields', async () => {
    const file = new File(['hello'], 'test.txt', { type: 'text/plain' })
    const node = await createDocumentNode(file, 'docid123', { x: 10, y: 20 }, 'hello world')
  ...
  it('should set status to error if extraction failed', async () => {
    const file = new File([''], 'fail.txt', { type: 'text/plain' })
    const node = await createDocumentNode(file, 'docid123', { x: 0, y: 0 }, '[Error extracting text from fail.txt]')
    expect(node.data.status).toBe('error')
  })

  it('should set status to processing if no extracted text', async () => {
    const file = new File([''], 'empty.txt', { type: 'text/plain' })
    const node = await createDocumentNode(file, 'docid123', { x: 0, y: 0 }, '')
