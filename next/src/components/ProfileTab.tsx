...
export default function ProfileTab() {
  ...
  // Cooldown state (optional; username_changed_at may not exist on all DBs)
            ...
            (() => {
              ...
              const Item = ({ n }: { n: any }) => {
                ...
                if (n.type === 'board_invite') {
                  ...
                  const fromUserId = p?.fromUserId || p?.otherUserId || null
                  const isRequest = /request/i.test(String(n.title || ''))
                  // Append Accept/Cancel links for pending requests
                  bodyNode = (
                    <span>
                      {typeof n.body === 'string' ? n.body : 'Connection update.'}
                      {isRequest && fromUserId && (
                        <>
                          {' '}
                          <button
                            className="text-primary-600 dark:text-primary-400 underline ml-2"
                            onClick={async () => {
                              try {
                                const me = user?.id
                                if (!me) return
                                await fetch('/api/connections', {
                                  ...
                                  body: JSON.stringify({ requesterId: fromUserId, addresseeId: me })
                                })
                                await markOneRead(n.id)
                                try { await ai.refresh() } catch {}
                                setBillingToast({ open: true, msg: 'Connection accepted', variant: 'success' })
                              } catch (e: any) {
                                setBillingToast({ open: true, msg: e?.message || 'Failed to accept', variant: 'danger' })
                              }
                            }}
                          >Accept</button>
                          ...
                          <button
                            ...
                            onClick={async () => {
                              try {
                                ...
                                await markOneRead(n.id)
                                setBillingToast({ open: true, msg: 'Connection request canceled', variant: 'success' })
                              } catch (e: any) {
                                setBillingToast({ open: true, msg: e?.message || 'Failed to cancel', variant: 'danger' })
                              }
                            }}
                          >Cancel</button>
                        </>
