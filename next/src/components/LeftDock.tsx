...
import { ListChecks, Tag as TagIcon } from '@phosphor-icons/react'

type DockKey = 'tasks' | 'colorgories' | null
...
export default function LeftDock({ active, onToggle }: LeftDockProps) {
  const baseBtn = "w-9 h-9 rounded-lg flex items-center justify-center transition-colors duration-150"
  ...
  return (
    <div
      className="fixed z-50 left-4 top-16 flex flex-col gap-2 p-2 rounded-xl border border-gray-200 dark:border-gray-700 bg-white/80 dark:bg-gray-900/80 shadow-lg backdrop-blur-sm"
      data-left-dock
